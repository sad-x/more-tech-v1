...
logging.basicConfig(filename=f"/home/alpha/scripts/logs/log_join_v2_medium__{datetime.strftime(datetime.now(), r'%y%m%d_%H%M%S')}.txt",
                    filemode='a',
...
spark.read.parquet(src).filter(col("eff_to_month") != lit("5999-12-31")).write.mode("append").partitionBy("eff_to_month", "eff_from_month").parquet(init_table)
log.info(f'insert (closed) in target finished by {(datetime.now() - tgt_insert_t0).seconds}s')
...
src_closed = src.filter(col("eff_to_month") != lit("5999-12-31")).select("id", "eff_from_month", "eff_from_dt")
src_closed.cache()
src.unpersist()
...
def run_task(from_dt):
    ...
    incr_open_single_part = src.filter(col("eff_to_month") == lit("5999-12-31")).filter(col("eff_from_month") == from_dt)
    all_open_single_part = tgt_no_match.union(incr_open_single_part)
    
    
    # tmp_part = f'{temp_table}/eff_to_month=5999-12-31/eff_from_month={from_dt}'
    tgt_open_part = f'{init_table}/eff_to_month=5999-12-31/eff_from_month={from_dt}'
    ...
    if not all_open_single_part.isEmpty():
        all_open_single_part.write.mode("overwrite").parquet(tgt_open_part)
    else:
        log.info(f"Found empty part {from_dt}, deleting")
        hadoop_conf = sc._jsc.hadoopConfiguration()
        fs = spark._jvm.org.apache.hadoop.fs.FileSystem.get(spark._jvm.java.net.URI(your_bucket), hadoop_conf)
        path = spark._jvm.org.apache.hadoop.fs.Path(tgt_open_part)
        fs.delete(path, True)
